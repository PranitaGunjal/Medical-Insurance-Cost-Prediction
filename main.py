...
if nav == 'Prediction':
    st.write("Calculating the INSURANCE CHARGES that could be charged by an Insure based on a Person's Attributes")
    st.markdown(' ------')
    ...
    def load_data():
        df1 = pd.DataFrame({'region': ['Southeast', 'Northwest', 'Southwest', 'Northwest']})
        return df1
    df1 = load_data()
